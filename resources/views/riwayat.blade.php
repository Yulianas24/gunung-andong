            ...
            <div class=" md:flex mb-2 p-3 justify-between bg-white rounded-md shadow-sm shadow-black/30 ">
                <div class="flex gap-1 py-1 ">
                    ...
                    <x-dropdown align="left">
                        <x-slot name="trigger">
                            <button
                                ...
                                {{ request('tahun') == null ? 'tahun' : request('tahun') }}
                                <i class="ri-arrow-down-s-line"></i>
                            </button>
                        </x-slot>
                        <x-slot name="content">
                            @for ($i = 2020; $i < (date("Y")); $i++)
                            <x-dropdown-link href="/riwayat?tahun={{ $i }}">
                                {{ __($i) }}
                            </x-dropdown-link>
                            @endfor
                        </x-slot>
                    ...
                    <x-dropdown align="left">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link href="/riwayat?status=sudah masuk">
                                {{ __('sudah masuk') }}
                            </x-dropdown-link>
                            <x-dropdown-link href="/riwayat?status=batal naik">
                                {{ __('batal naik') }}
                            </x-dropdown-link>
                        </x-slot>
                    </x-dropdown>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                @else
                                <button
                                class="py-[2px] px-3 justify-between  rounded-full text-[11px]  flex  {{ $data->status == 'sudah masuk' ? 'bg-green-200' : ($data->status == 'belum masuk' ? 'bg-yellow-200' : 'bg-red-200') }} ">
                                    {{ $data->status }}
                                </button>
                                @endif
